...
| 中性通道板块行情护栏 | ✅ 2026-07-17 | 发现 44 条回补中性 case 全挤在 2026-07-03~10 一周内，其中 `valuation/flow 缺失` 两桶更是同一天快照（伪信号）。新增 ISO 周分散度护栏 `neutral_min_periods=2`：中性模式需跨 ≥N 个不同 ISO 周才晋级， 一次性板块/单日事件不再被铸成永久经验；真实再跑 `neutral_regime_filtered:2`、`lessons_deactivated:2`，三条 → 一条（仅保留跨 2 周的地产 high）。**行业指数 beta 分解已落地**：中性 case 计 `sector_excess_return`（个股−行业指数），行业桶优先用 sector 超额分离「选股规避」与「板块普跌」，payload 记 `basis`/`sector_adjusted_n` |
...
0. ~~**中性通道加「板块性行情」护栏**~~ 已实现时间分散度护栏（`neutral_min_periods=2`，跨 ISO 周才晋级，已滤掉单日 valuation/flow 伪信号）。~~**剩余 TODO：行业指数 beta 分解**~~ **已实现**——反思引擎为中性 case 额外计算 `sector_excess_return`（个股收益 − 同期行业指数收益，行业经 `normalize_industry` 归一后映射到 SW 一级行业指数，`reflection_industry_index_map` 可覆盖）；miner 行业桶优先用 sector 超额、缺指数时按 case 回退大盘超额并在 payload 记 `basis`（`sector_adjusted`/`mixed`/`broad`）+`sector_adjusted_n`，finding 文案随之标注「相对行业指数」。板块整体普跌但个股仅跟随（sector 超额≈0）的桶不再被误铸为选股经验，只有跑输自身行业的个股才晋级。best-effort 降级：无映射/取数失败时等价于原大盘超额行为。
...
2. ~~为 DailyPipeline 增加可选 Top N 深度 StockAnalysisSkill 串联，并把深度结论回写 candidate payload。~~ **已实现**——量化排名 + LLM 复核后，可选对 Top N 候选跑重量级多智能体 `StockAnalysisSkill`，把 `structured_conclusion`（rating/target_price/confidence/reasons/plan）回写到候选 payload 的 `deep_analysis` 字段，signal 行、选股卡片、反思快照均携带该深度结论；量化+LLM 的入选理由作为 `selection_context` 传入深度分析。默认关闭（`daily_pipeline_deep_analysis_enabled`，`daily_pipeline_deep_analysis_limit=1`），因每只跑完整 agent graph（分钟级 + token）；best-effort，深度分析失败/为空不阻断流水线。
3. ~~扩展 StrategyBacktest 的 walk-forward/ablation 对比和执行滑点明细，不增加任意参数搜索器。~~ **已实现**——`audit_backtest_result` 为 `validation` 附加三块**只读、非门禁、best-effort** 摘要：`walk_forward`（纯后处理 purged CV——折次 sharpe 一致性、正收益折次占比、样本内→样本外衰减，不跑额外回测、不搜参）、`execution_slippage`（结果带成交明细时才调 MCP）、`ablation_study`（仅当调用方**显式声明** base experiment + ablation 列表时运行，非参数搜索）。输入缺失时各自降级为 `{"available": false, ...}`。
4. ~~扩展 Playwright 到真实后端的预发布 smoke；CI 合约 mock 负责稳定验证前端主流程。~~ **已实现**——新增 env 门控 smoke（`E2E_REAL_BACKEND=1`、`frontend/e2e/*.smoke.ts`、`npm run test:e2e:smoke`），经 Playwright webServer 同时启动真实 FastAPI（临时 DB、MCP 降级）与 vite dev，端到端断言前端外壳与后端连通（健康探针 + 外壳渲染 + 路由跳转）。默认 `npm run test:e2e` 仍只跑 mock 的 `*.spec.ts` 合约，作为稳定的前端主流程验证器；独立 `e2e-smoke` CI job 跑真实后端。
