...
export default function Audit() {
  ...
  return <div className="mx-auto flex max-w-7xl flex-col gap-5">
    ...
    <section className="rounded-lg border border-stone-800 bg-stone-900 p-4">
      ...
      <div className="mt-3 space-y-2">{(backtests.data ?? []).map(b => <div key={b.id} className="grid gap-2 rounded border border-stone-800 p-3 text-xs text-stone-300 sm:grid-cols-6"><span>{b.start_date} → {b.end_date}</span><span>收益 {pct(Number(b.result.total_return))}</span><span>回撤 {pct(Number(b.result.max_drawdown))}</span><span>Sharpe {numberText(b.result.sharpe)}</span><span>{String((b.result.validation as Record<string, unknown> | undefined)?.production_gate_passed ?? false) === "true" ? "门禁通过" : "门禁未通过"}</span><span>{b.status}</span></div>)}</div>
    </section>
  </div>;
}
